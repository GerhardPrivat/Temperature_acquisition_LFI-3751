...
if (count != countmax):
    data_np_copy = np.zeros(shape=(2,count), dtype="float")
    data_np_copy[0] = np.copy(data_np[0][:count])
    data_np_copy[1] = np.copy(data_np[1][:count])
    np.savetxt(storagepath+"Temperature_%d_%02d_%02d_%02d_%02d.txt" % (now.year,now.month,now.day,now.hour,now.minute), np.transpose(data_np_copy), fmt="%.3f", delimiter=",", newline="\n")        
else:
    np.savetxt(storagepath+"Temperature_%d_%02d_%02d_%02d_%02d.txt" % (now.year,now.month,now.day,now.hour,now.minute), np.transpose(data_np), fmt="%.3f", delimiter=",", newline="\n")        
...
ax.plot(data_np[0][:count],data_np[1][:count])
ax.set_xlim(np.amin(data_np[0][:count])-0.5,np.amax(data_np[0])+0.5)
ax.set_ylim(np.amin(data_np[1][:count])-0.5,np.amax(data_np[1])+0.5)
